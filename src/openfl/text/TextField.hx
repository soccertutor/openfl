	...
	**/
	...
	@:noCompletion private function stage_onMouseMove(event:MouseEvent):Void
	{
		...
		if (selectable && __selectionIndex >= 0)
		{
			...
			if (__lineSelection)
			{
				position = __getPositionByIdentifier(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY, true);
			}
			else if (__wordSelection)
			{
				position = __getPositionByIdentifier(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY, false);
			}
			else
			{
				position = __getPosition(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY);
			}
	...
	@:noCompletion private function stage_onMouseUp(event:MouseEvent):Void
	{
		...
		if (stage.focus == this)
		{
			...
			if (__lineSelection || __wordSelection)
			{
				...
				if (__lineSelection)
				{
					upPos = __getPositionByIdentifier(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY, true);
				}
				else if (__wordSelection)
				{
					upPos = __getPositionByIdentifier(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY, false);
				}
