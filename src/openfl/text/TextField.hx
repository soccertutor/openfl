	...
	**/
	...
	@:noCompletion private function stage_onMouseMove(event:MouseEvent):Void
	{
		...
		if (selectable && __selectionIndex >= 0)
		{
			...
			var position = if (__lineSelection) __getPositionByIdentifier(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY,
				true) else if (__wordSelection) __getPositionByIdentifier(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY,
				false) else __getPosition(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY);
	...
	@:noCompletion private function stage_onMouseUp(event:MouseEvent):Void
	{
		...
		if (stage.focus == this)
		{
			...
			var upPos:Int = if (__lineSelection) __getPositionByIdentifier(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY,
				true) else if (__wordSelection) __getPositionByIdentifier(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY,
				false) else __getPosition(mouseX + (textDirection.backward ? -scrollH : scrollH), mouseY);
			var leftPos:Int;
			var rightPos:Int;
	...
	@:noCompletion private function this_onMouseDown(event:MouseEvent):Void
	{
		...
		if (__lineSelection)
		{
			...
			__caretIndex = __getPositionByIdentifier(event.stageX + (textDirection.backward ? -scrollH : scrollH), event.stageY, true);
		...
		{
			var prevCaretIndex = __caretIndex;
			__caretIndex = __getPositionByIdentifier(event.stageX + (textDirection.backward ? -scrollH : scrollH), event.stageY, false);
